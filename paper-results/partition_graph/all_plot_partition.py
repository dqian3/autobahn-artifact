...
bullshark_start_cutoff = 3
bullshark_end_cutoff = 10
bullshark_offset = 2

batch_hs_start_cutoff = 2
batch_hs_end_cutoff = 11
batch_hs_offset = 3

batch_hs_start_cutoff2 = 2
batch_hs_end_cutoff2 = 11
batch_hs_offset2 = 3
...
adjust_file(batch_hs_file_name2, batch_hs_offset2)
...
plt.scatter(np.array(x), np.array(y), s=10, color='red', label='Vanilla HS',marker='d')
plt.plot(x, y, '.r-')
...
plt.scatter(np.array(x_motor), np.array(y_motor), s=20, color='green', label='Autobahn', marker='o')
plt.plot(x_motor, y_motor, '.g-')
print('Motorway')
print(y_motor[28:])
proc_motor = y_motor[28:]
motor_mean = sum(proc_motor) / len(proc_motor)
print(motor_mean)
...
print(y_bs[34:])
proc_bs = y_bs[34:]
bs_mean = sum(proc_bs) / len(proc_bs)
print(bs_mean)

print(bs_mean / motor_mean)

plt.scatter(np.array(x_bs), np.array(y_bs), s=10, color='blue', label='Bullshark', marker='v')
...
plt.scatter(np.array(x_batch), np.array(y_batch), s=10, color='orange', label='Batched HS', marker='s')
plt.plot(x_batch, y_batch, color='orange', ls="dashed") #'.m-.')

##75k
df_batch = pd.read_csv(batch_hs_file_name2 + '-processed.txt', sep=',', header=None)
df_batch.columns = ["start", "end", "latency"]
df_batch['start'] = pd.to_timedelta(df_batch['start'], unit='s')
#df['end'] = pd.to_timedelta(df['end'], unit='s')
plot_df_batch = df_batch.groupby(pd.Grouper(key='start', freq='1s')).mean()
#print(plot_df.index.get_level_values('start'))
x_batch = plot_df_batch.index.get_level_values('start') / np.timedelta64(1, 's')
x_batch = x_batch.to_list()[batch_hs_start_cutoff2:-batch_hs_end_cutoff2]
y_batch = plot_df_batch.get('latency').to_list()[batch_hs_start_cutoff2:-batch_hs_end_cutoff2]

#plt.scatter(np.array(x_batch), np.array(y_batch), s=10, color='purple', label='Batch HS (75k)', marker='s')
...
#plt.scatter(np.array(x_motor), np.array(y_motor), s=10, color='green', label='Autobahn (75k)', marker='x')
#plt.plot(x_motor, y_motor, '.g--')


plt.vlines(x=[async_start+0.5,async_end+0.5], ymin=0, ymax=23, colors='purple', ls='--', lw=2.5, label='Blip Duration')
#plt.vlines(x=[hangover_end], ymin=0, ymax=23, colors='blue', ls='dotted', lw=2.5, label='HS Hangover')
