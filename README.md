...
> Unfortunately, trial access users *cannot* access the machine type used in our evaluation, and must instead use a weaker machine type (more details below). We have NOT thoroughly evaluated or systems on these machine types, but provide [sample runs that validate our core results](#freemachines). To accurately reproduce our *paper* results, we recommend using the same machine types employed in our experiments, and using the SPOT-market to save costs. Re-running only our key data points, using the spot market, should be relatively cheap -- but make sure to terminate machines as soon as you do not need them. Failing this, please send an anonymous comment, we are investiagting ways to securely add you to our own GCP account and specify an allowance.
...
The configs and associated results can be found in `experiment_configs/blips-graph/`. 
> [!NOTE] 
...
The configs and associated results can be found in `experiment_configs/partition_graph/`
> [!NOTE] 
